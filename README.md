# Portfolio

Welcome to my portfolio repository.

This repository is a collection of non-professional projects that I have worked
on. These projects where started as part of my personal development to try and learn new
programming skills and languages.

## Projects

Each project has its own folder and documentation, currently the code in this repository are snippets
of the complete code from the original projects as they are still work in progress 
or contain data that I don't want in a public repository (yet).

The projects included in this repository are:
- [A website for VIN decoding](Projects/VinDecodingWebsite/README.md)
- A football game database project and analytics

## Used Technologies
Below is displayed which technologies are used:
- Python ([VIN decoding website](Projects/VinDecodingWebsite/README.md))
  - Flask ([VIN decoding website](Projects/VinDecodingWebsite/README.md))
  - Pandas
- MySQL
- Tableau
